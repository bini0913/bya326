...
function GalleryAdmin() {
  ...
  const load = async () => {
    ...
    const withUrls = (data ?? []).map(
      (r: { id: string; storage_path: string; caption: string | null; sort_order: number }) => ({
        ...r,
      ...
      {loading ? (
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {items.map((img) => (
            <div
              key={img.id}
              className="group overflow-hidden rounded-sm border border-navy-900/10 bg-white"
            >
              <div className="relative aspect-square">
                ...
                <button
                  onClick={() => del(img)}
                  className="absolute right-2 top-2 rounded-sm bg-white/90 p-1.5 opacity-0 transition-opacity group-hover:opacity-100"
                >
