  ...
  useEffect(() => {
    void (async () => {
      const [a, an, np, nd, g, recent] = await Promise.all([
        ...
        supabase
          .from("admissions")
          .select("id, ref_id, parent_name, grade, status, created_at")
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          icon={Inbox}
          label="New Inquiries"
          value={stats?.admissionsNew}
          sub={`${stats?.admissionsTotal ?? 0} total`}
          to="/admin/admissions"
        />
        <StatCard
          icon={TrendingUp}
      ...
      <div className="mt-10 rounded-sm border border-navy-900/10 bg-white p-6">
        <div className="flex items-center justify-between">
          ...
          <Link
            to="/admin/admissions"
            className="text-xs font-semibold uppercase tracking-wider text-gold-600 hover:text-gold-500"
          >
            View all →
          </Link>
        </div>
            ...
            {stats?.recentAdmissions.map((r) => (
              <li key={r.id} className="flex items-center justify-between py-3 text-sm">
                ...
                <div className="text-right">
                  <span className="rounded-full bg-cream px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-navy-900">
                    {r.status}
                  </span>
...
function StatCard({
  ...
  return (
    <Link
      to={to}
      className="block rounded-sm border border-navy-900/10 bg-white p-5 transition-shadow hover:shadow-md"
    >
      <div className="flex items-center justify-between">
